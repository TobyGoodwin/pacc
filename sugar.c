...
static void deblit1(struct s_node *g, struct s_node *n) {
    char *gd, *id;
    int l;
    struct s_node *p, *q;
...
static void derep1(struct s_node *g, struct s_node *n, char *name) {
    char *x0, *x1, *xx;
    int l;
    ...
    l = snprintf(0, 0, "%s:.", name) + 1;
    x0 = malloc(l);
    if (!x0) nomem();
    snprintf(x0, l, "%s:.", name);
    //fprintf(stderr, "!!! got a rep - nm is %s !!!\n", x0);
    x0[l - 2] = '1'; x1 = strdup(x0); if (!x1) nomem();
    x0[l - 2] = '*'; xx = strdup(x0); if (!xx) nomem();
    free(x0);
